<!DOCTYPE html>
<HTML>
	<HEAD>
		<TITLE>Warhammer Online Battle</TITLE>
		<LINK href="css/home.css" rel="stylesheet" type="text/css">
	</HEAD>
	<BODY>
		<img class="float_img" src="http://media.moddb.com/images/groups/1/3/2055/469px-Storm_Warden.png">
		<center>
			<div id="global">
				<div id="log_bar">
					<table border="0">
						<tr>
							<td class="header_cell"><a href="login.html" title="">LOGIN</a></td>
							<td class="header_cell"><a href="login.html" title="">REGISTER</a></td>
							<td class="header_cell"><a href="" title="">ABOUT</a></td>
						</tr>
					</table>
				</div>
				<a href="index.php" title="Home"><span id="header_title">[HOME:PAGE]</span></a>
				<div id="global_block">
					<div class="menu_block">
						<span class="menu_block_typo">[LAUNCH]<img class="img_block" src="http://mmorpg-tactics.com/wp-content/uploads/2012/03/warhammer-online-logo-1812d25.png"></span>
					</div>
					<div class="menu_block">
						<a href="multi.php" title="multi"><span class="menu_block_typo">[MULTI]<img class="img_block" src="http://www.fantasyflightgames.com/ffg_content/warhammer_lcg/empire-icon.png"></span></a>
					</div>
					<div class="menu_block">
						<a href="profile.php" title="profile"><span class="menu_block_typo">[PROFILE]<img class="img_block" src="http://img1.wikia.nocookie.net/__cb20111205015944/warhammer40k/images/a/ad/Calixis-logo.png"></span></a>
					</div>
					<div class="menu_block">
						<span class="menu_block_typo">[LOBBY]<img class="img_block" src="http://2.bp.blogspot.com/-wn8YsUC-o1I/TQrHQO3QPwI/AAAAAAAAANo/ElW0ZOLE2FY/s320/blood+angels+warhammer.png"></span>
					</div>
				</div>
			</div>
		</center>
	</BODY>
</HTML> 
